...
void imprime_matriz(int A[3025][16]);
void promedio_images(int A[3025][16]);
Mat vec_2_img();
...
int main(int argc, char** argv){
  ...
  namedWindow( "img_Promedio", WINDOW_NORMAL); //normal para modificar ventana a gusto
  resizeWindow( "img_Promedio", 550, 400);
  imshow("img_Promedio", imProm);
...
void Img_2_vector(int cont, Mat Img_aux){
  ...
  for( int i = 0; i < nRen; i++ ){
    ...
    for( int j = 0; j < nCol; j += nCanales ){
      //if (*( renglon + j) == 255 )
        Vec_img[tamx*j+i][cont] = *( renglon + j);
      //else Vec_img[100*j+i][cont] = 0;
    }
  }
}
